...
def is_clean_value(value):
    return abs(value - round(value)) < 0.0001 or is_quarter_increment(value)
...
def break_down_to_clean_units(base_amount, unit_conversions, measurement_type):
    ...
    for unit_fact in units_sorted:
        ...
        if amount_in_unit >= 1.0 and is_clean_value(amount_in_unit):
    ...
    if remaining > 0.0001:
        ...
        if is_clean_value(final_amount):
    ...
    all_clean = all(is_clean_value(c['amount']) for c in components)
...
def calculate_optimal_unit(bindings, wm, kb):
    ...
    components = break_down_to_clean_units(base_amount, unit_conversions, measurement_type)
